...
const parseXmlContent = (xmlContent: string): { title: string; items: string[] } | null => {
    // エンコードされた患者管理用XMLタグを検出
...
const EventDetailsModal: React.FC<EventDetailsModalProps> = ({ event, onClose }) => {
    ...
    const calendarName = event.calendarId && CALENDAR_NAMES[event.calendarId]
        ...
        : 'カレンダー';
    const replacedDescription = decodeEntities(event.description ?? '')
    // 説明文からXMLコンテンツを解析
    const xmlData = event.description ? parseXmlContent(replacedDescription) : null;
                        ...
                        {xmlData && (
                            <div className="flex">
                                ...
                                <div className="ml-2 w-full">
                                    <h3 className="font-medium text-gray-700 mb-2">{xmlData.title}</h3>
                                            ...
                                            {xmlData.items.map((item, index) => {
                                                ...
                                                return (
                                                    <li key={index} className="flex items-start py-1 border-b border-gray-100 last:border-b-0">
                                                        <span className="font-medium w-1/3 text-gray-700">{displayName}</span>
                                                        <span className="w-2/3">{fieldValue}</span>
                                                    </li>
                                                );
                                            })}
                                        </ul>
